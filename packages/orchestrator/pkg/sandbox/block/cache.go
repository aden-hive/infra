...
func (c *Cache) WriteAtWithoutLock(b []byte, off int64) (int, error) {
	...
	if int64(len(b)) < c.blockSize || off%c.blockSize != 0 {
		return 0, fmt.Errorf("WriteAtWithoutLock requires block-aligned write: len=%d off=%d blockSize=%d", len(b), off, c.blockSize)
	}

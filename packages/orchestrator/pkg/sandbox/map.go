...
func (m *Map) LifecycleItemsByState(states ...SandboxState) []*Sandbox {
	...
	sandboxes := make([]*Sandbox, 0, len(entries))
	for _, entry := range entries {
		if _, ok := stateSet[entry.state]; !ok {
			continue
		}
...
func (m *Map) TrackLifecycle(ctx context.Context, sbx *Sandbox, state SandboxState) {
	m.lifecycles.Insert(sandboxLifecycleKey(sbx.Runtime.SandboxID, sbx.LifecycleID), lifecycleEntry{
		sandbox: sbx,
		state:   state,
	})
...
func (m *Map) MarkRunning(ctx context.Context, sbx *Sandbox) {
	m.markLifecycleState(sbx.Runtime.SandboxID, sbx.LifecycleID, SandboxStateRunning)
